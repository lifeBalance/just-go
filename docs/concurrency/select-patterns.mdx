...
In our previous [select statements](../statements/select-statements) section, we saw the basic usage of `select` with channels. Here, we'll explore some common patterns and advanced usage of `select` in Go.
...
func main() {
    ...
    default:
        fmt.Println("No message received") // ✅ No message received (channel has no data)
    }
}
```
