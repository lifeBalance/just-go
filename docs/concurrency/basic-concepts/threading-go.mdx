...
In a previous section we talked about **kernel threads** and **user-level threads**. The latter ones are **lightweight threads** that are scheduled by a runtime library or virtual machine (VM). Depending on the language they have different names:

- [Green threads](https://en.wikipedia.org/wiki/Green_thread) is the name of the original thread library for Java.
- [Greenlets](https://pypi.org/project/greenlet/) are lightweight coroutines for in-process concurrent programming (Python library).
- [Goroutines]() are Go’s lightweight user threads multiplexed onto kernel threads by the runtime.

All these have in common several things:

- They’re scheduled in **user space** by a **runtime** (Go) or **library** rather than the **OS scheduler**. 
