...
A [process](<https://en.wikipedia.org/wiki/Process_(computing)>) is the instance of a computer program that is being executed. For example, [sleep](<https://en.wikipedia.org/wiki/Sleep_(command)>) is a command that as soon as it starts, suspends execution for the amount of time we pass as argument. So when we run:
