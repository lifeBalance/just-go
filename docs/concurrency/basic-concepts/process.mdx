...
A [process](<https://en.wikipedia.org/wiki/Process_(computing)>) is the instance of a computer program that is being executed. 

> [!NOTE]
> A **program** is is a set of instructions intended to be executed. These instructions are executable files on disk. A **process** is a running instance of a program created by the operating system.

For example, [sleep](<https://en.wikipedia.org/wiki/Sleep_(command)>) is a command that as soon as it starts, suspends execution for the amount of time we pass as argument. So when we run:
...
<Admonition title="Jobs vs Processes">
  A [job](https://en.wikipedia.org/wiki/Job_control_(Unix)) is a higher-level
  concept that represents a group of one or more processes. Jobs are managed by
  the shell, while processes are managed by the operating system.
</Admonition>
...
│ │ 0 stdin  1 stdout   ││        │ │ 0 stdin  1 stdout   ││
│ │ 2 stderr 3 socket#17││        │ │ 2 stderr 4 pipe#42  ││
│ └─────────────────────┘│        │ └─────────────────────┘│
└────────────────────────┘        └────────────────────────┘
```
...
In the **memory space**, we can find several segments:

- [Text Segment](https://en.wikipedia.org/wiki/Text_segment): Contains the executable code of the program.
- [Data Segment](https://en.wikipedia.org/wiki/Data_segment): Contains global and static variables
- [Heap Segment](https://en.wikipedia.org/wiki/Heap_(data_structure)): Used for dynamic memory allocation, where variables are allocated and freed at runtime.
- [Stack Segment](https://en.wikipedia.org/wiki/Call_stack): Used for function call management, local variables, and control flow.

## Inter-Process Communication (IPC)

When processes need to communicate with each other they must share parts of their address spaces or use other forms of [inter-process communication (IPC)](https://en.wikipedia.org/wiki/Inter-process_communication). 
...
│ └─────────┘ │                        │ └─────────┘ │
│ ┌─────────┐ │                        │ ┌─────────┐ │
│ │ msg send│─┼── Message Queue ───────┼─│ msg recv│ │
│ └─────────┘ │                        │ └─────────┘ │
│ ┌─────────┐ │                        │ ┌─────────┐ │
│ │ shm map │─┼── Shared Memory ───────┼─│ shm map │ │
│ └─────────┘ │                        │ └─────────┘ │
│ ┌─────────┐ │                        │ ┌─────────┐ │
