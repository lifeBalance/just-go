...
In the section about [processes](/concurrency/basic-concepts/process) we mention that they can communicate with each other using [inter-process communication (IPC)](https://en.wikipedia.org/wiki/Inter-process_communication).

Sometimes, **threads** working together to solve a problem, also need to interchange information. There are two ways of doing this:
...
That means one thread can update data structures that another thread reads a moment later, but that convenience comes with some hazards. Without coordination, two threads writing the same variable can trample each other’s updates, leading to several issues such as:
