...
```
Time │ Goroutine 1 (deposit)  │ Goroutine 2 (withdraw) │ money
─────┼────────────────────────┼────────────────────────┼──────
  1  │ READ (100)             │                        │ 100
  2  │ ADD 10: 110            │                        │ 100
  3  │                        │ READ (100)             │ 100
  4  │                        │ SUBTRACT 10: 90        │ 100
  5  │ WRITE 110.             │                        │ 110
  6  │                        │ WRITE money: 90        │ 90  ← Lost deposit 10!
```
...
─────┼────────────────────────┼────────────────────────┼──────
  ...
  4  │                        │ WRITE 90               │ 90
  5  │ ADD 10                 │                        │ 90
  6  │ WRITE money: 110       │                        │ 110 ← Lost withdrawal 10!
```
...
│  (Manages goroutines on OS threads) │
└─────────────────────────────────────┘
          │                  │
...
In a [previous secction](/basic-concepts/scheduling#preemptive-scheduling) we saw how the **OS scheduler** uses [preemptive scheduling](https://en.wikipedia.org/wiki/Preemption_(computing)), meaning it would interrupt a running task and hand the CPU to another one, even if the running task hasn’t blocked or yielded.
