...
The most interesting test here is the `TestLookupIncrementError`, since I had to stub the `Storage` so I could add an `IncrementHits` method that returns an error (otherwise it couldn't be hard to test this).
