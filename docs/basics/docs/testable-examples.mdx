...
The main point of testable examples is to make sure that the examples that we add in our docs, stay in sync with the code they try to explain. A good way of accomplishing that is to treat them as a sort of tests that will fail if they fall out of sync. Everytime we **test** our code, our **examples** will also be tested, to make sure that the **output** is correct.
...
<Admonition type="important" title="Naming Examples">
The name after the `Example*` prefix will determine where in the docs, the example will show up (package level, type level, etc).
</Admonition>

The `ExampleGreeter_Greet` means that the example will show up under the method `Greet` of the type `Greeter`. If you want to add more examples in the same place, add lowecase suffixes such as `ExampleGreeter_Greet_default`, or `ExampleGreeter_Greet_whatevs`. Lowercase words after the `_` will be used to name the examples.
