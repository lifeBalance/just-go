...
In Go, there are no prefix versions of the increment (`++`) and decrement (`--`) operators. They can only be used as postfix operators. So using `++i` or `--i` would generate a **compile error**.
</Admonition>
