...
Knowing these **precedence rules** only matters when you **mix** bitwise and arithmetic without parentheses. Mixing **arithmetic** and **bitwise** operators is common in systems work, parsing, compression, graphics, hashing/cryptography, and performance‑critical code. It’s much less common in everyday business logic.
...
Note: right shift is arithmetic for signed integers (sign bit propagates) and logical for unsigned (zeros shift in). Prefer unsigned if you need zero‑fill behavior.
...
Bitwise **AND** (`&`) and **AND NOT** (`&^`) group with arithmetic multiplication and remainder because they behave similarly in expressions.
...
The `&` operator sets a bit when both input bits are 1.

```go
u := 6 & 3  // 2
```

Visualizing with binary integers:

```go
x := uint8(0b0110)          // 0 1 1 0 👉 6
y := uint8(0b0011)          // 0 0 1 1 👉 3
fmt.Printf("%04b\n", x&y)   // 0 0 1 0 👉 2
```

### Bit clear (AND NOT)

The `&^` operator clears bits in the left operand that are set in the right.
...
```go
p := uint8(0b0110)           // 0 1 1 0 👉 6
q := uint8(0b0011)           // 0 0 1 1 👉 3
fmt.Printf("%04b\n", p&^q)   // 0 1 0 0 👉 4
```

## Bitwise OR and XOR is like Addition/Subtraction

Bitwise OR (`|`) and XOR (`^`) share the additive group with `+` and `-`. They associate left‑to‑right, so mixed expressions without parentheses parse the same way.
...
If you visualize binary integers, it’s clearer:

```go
x := uint8(0b0010)          // 0 0 1 0 👉 2
y := uint8(0b0100)          // 0 1 0 0 👉 4
fmt.Printf("%04b\n", x|y)   // 0 1 1 0 👉 6
```
