...
<Admonition type="tip" title="Binary Integer literals">
  ...
  readability (e.g., `0b1111_0000`). See the spec’s [Integer
...
x := uint8(0b1100)          // 1100
y := uint8(0b1010)          // 1010
fmt.Printf("%04b\n", x&y)   // 1000 👈 Check the 2nd and 3rd bits
```
...
<Admonition variant="tip" title="Bit clear (&^)">
  `a &^ b` clears any bits in `a` that are set in `b`. It’s handy for unsetting
  flags.
</Admonition>
