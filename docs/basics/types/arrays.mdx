...
The statement above declares an array of integers named `numbers` with four elements inside. All the elements in this array are initialized to the zero value of the `int` type, which is `0`. This is the in memory layout of this array:
...
We can also declare and initialize an array in a single line using a [composite literal](https://go.dev/ref/spec#Composite_literals):
