...
import sliceCode11 from './code/slices/slices11/main.go?raw'
import sliceCode12 from './code/slices/slices12/main.go?raw'
import sliceCode5 from './code/slices/slices5/main.go?raw'
import sliceCode13 from './code/slices/slices13/main.go?raw'
import sliceCode14 from './code/slices/slices14/main.go?raw'

import sliceCode1 from './code/slices/slices1/main.go?raw'
import sliceCode2 from './code/slices/slices2/main.go?raw'
import sliceCode3 from './code/slices/slices3/main.go?raw'
import sliceCode4 from './code/slices/slices4/main.go?raw'
import sliceCode6 from './code/slices/slices6/main.go?raw'
import sliceCode7 from './code/slices/slices7/main.go?raw'
import sliceCode8 from './code/slices/slices8/main.go?raw'
import sliceCode9 from './code/slices/slices9/main.go?raw'
import sliceCode10 from './code/slices/slices10/main.go?raw'
...
In the code above, we're starting with an **empty slice**, and add some elements using the `append` built-in function. In those cases where we start with a small amount of elements which are known beforehand, we could also specify them in the composite literal:
...
```go
a := [...]byte{}  // Declaring empty array
s := a[:]         // Declaring empty slice
```

To expand a **not initialized** slice you can use `append` (since it creates an underlying array if need be) but never a **slicing operation**, otherwise you'd get a panic error.

## Creating Empty Slices

Creating **empty slices** is extremely common in Go, it's a core idiom for handling collections that start empty but may grow according to our program needs. There are several ways to create an empty slice:

- Using a composite literal.
- Using the `make` function.
- Using a slicing expression.
...
In the previous example we used a strange syntax to create a slice out of an array:

```go
s1 := a[:]
```

This is known as a [slice expression](https://go.dev/ref/spec#Slice_expressions), which in its general form, it's as follows:
...
A slice always refer to an underlying array, but it's not just a **pointer**. Check the following code:

<Code code={sliceCode1} lang="go" title="slices1/main.go" />

Here we have created an **array** of 7 integers but only have initialized 4 of them. Then we have created a **slice** out of that array. Several things to notice here:
...
The documentation refers to slices as **descriptors**, small data structures holding three pieces of information. The following diagram illustrates the slice of our last example:
...
<Admonition>
  ...
  have. In our example, the slice `s1` has `3` elements, but we could extend it up to `5`.
</Admonition>
...
## Resizing slices: When references break
