...
<Admonition>
Basically is like declaring an **array**, the only difference is the missing length (or the `...`) between the square brackets.
</Admonition>
...
In the code above, we're starting with an **empty slice**, and adding elements according to our needs (an extremely common pattern in Go). But if we starting with a small amount of elements which are known beforehand, we could also specify them in the composite literal:
...
So far we've seen how we can create slices using `var` syntax and the short syntax. Actually, there's a difference:
...
This syntax implicitly creates the underlying array for us big enough to hold the specified items (that's why the **length** and **capacity** of slices created this way will always be the same).
...
If we need to extend a slice beyond the capacity limit we can use the [append](https://pkg.go.dev/builtin#append) built-in function. Using this function the resulting slice may or may not refer to the original array:

- If it has sufficient capacity, the slice is extended, and refers to the original array.
