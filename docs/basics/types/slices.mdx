...
When we are dealing with a small amount of elements which are known beforehand, we could specify them in a [composite literal](https://go.dev/ref/spec#Composite_literals):

<Code code={sliceCode5} lang="go" title="slices5/main.go" />

Basically is like declaring an array, the only difference is the missing **length** between the **square brackets**. This syntax implicitly creates the underlying array for us big enough to hold the specified items (that's why the **length** and **capacity** of slices created this way will always be the same).
