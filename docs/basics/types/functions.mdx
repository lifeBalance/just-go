...
[Functions](https://go.dev/ref/spec#Function_types) in Go work, for the most part, like in any other programming language.

## How Functions Work

A function is declared with the `func` keyword, a **name**, parameters (with types), and optional return types. For example:
...
The function above is named `add`, it takes two parameters of type `int`, and it returns a single `int` value (the addition of the two parameters).

<Admonition type="info" title="Parameters">
  Parameters are defined as a list of names followed by their type. When
  multiple parameters share the same type, you can omit the type for all but the
  last one.
</Admonition>

You **call the function** by passing **arguments** that match its **parameters**:

```go
result := add(2, 3) // result is 5
```

Functions can also return multiple values. For example, a function that performs integer division and returns both the quotient and the remainder:

```go
func divisionRemainder(n, d int) (int, int) {
    return n/d, n%d
}
```

Optionally, you can name the return values in the **function signature**:
...
<Admonition type="info" title="Return types">
  Return types appear after the parameter list; use parentheses when returning
  multiple values.
</Admonition>

You can call this function and capture both return values:

```go
q, r := divisionRemainder(7, 3) // q is 2, r is 1
```

Functions can also have **no return values** at all:
...
In Go, we can define functions that can take variable number of arguments:

<Code code={functionsCode6} lang="go" title="functions6/main.go" />

If your function is gonna take more arguments, it's essential you add the variadic arguments at the end. Ok, but what if we have all the numbers in a slice? Well, you can still use your variadic function, just make sure you unpack the slice items:
...
This technique is a bit hard to find in the docs, but it's [there](https://go.dev/ref/spec#Passing_arguments_to_..._parameters)!
...
Functions commonly return a value plus an `error`. [Errors](https://go.dev/ref/spec#Errors) are ordinary values; callers should check and handle them.
