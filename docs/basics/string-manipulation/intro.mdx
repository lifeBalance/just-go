...
import concatNaive from './code/concat-naive/main.go?raw'
import concatBuilder from './code/concat-builder/main.go?raw'
import removeNth from './code/removeNth/main.go?raw'

# String Manipulation

String manipulation is ubiquitous in Go — parsing input, formatting messages, building URLs, handling JSON, and talking to APIs. This section focuses on practical string operations and patterns.
...
<Code code={concatNaive} lang='go' />

This code outputs:

```
naive: len=100000 time=298.739583ms 🐌 Slow (milliseconds)
```
...
<Code code={concatBuilder} lang='go' />

The output of this one:

```
builder: len=100000 time=500.125µs 🚀 Fast! (microseconds)
```
...
func main() {
	...
	fmt.Println(strings.Title("gopher"))    // Gopher
}
```

[Title](https://pkg.go.dev/strings#Title) capitalize its string argument.
...
## Example: Remove the Nth Occurrence of a Substring

Let's write a function to remove only the nth occurrence of a substring.

<Code code={removeNth} lang='go' />

## Comparing and Sorting Strings

Comparing and ordering strings comes up often in programming contexts like filters, sorted lists, and UI rendering. In Go, strings are compared [lexicographically](https://en.wikipedia.org/wiki/Lexicography) by their byte values in UTF-8 encoding, which has important implications for correctness and performance.

<Admonition title='Lexicographically'>
Lexicographically means in dictionary order, or like words are listed in a dictionary (A to Z, comparing letter by letter), and it's used in computing and math to sort things (words, numbers, data) character by character, just as you'd find them in a dictionary. For example, `apple` comes before `apply` because, after all the common letters (`appl`) `e` comes before `y`.
</Admonition>

### Basic Comparison

Go's comparison operators (`==`, `!=`, `<`, `>`, `<=`, `>=`) perform byte-by-byte comparisons:

```go
fmt.Println("absence" < "axe")  // true
fmt.Println('b')                // 98
fmt.Println('x')                // 120
```

The first letters of both words are the same, then the comparison moves to the next byte, and since the byte value of `b` (`98`) is less than the value of `x` (`120`), the first word is lexicographically smaller than the second one.

### Dedicated Function

Go provides a dedicated comparison function in the [strings](https://pkg.go.dev/strings) package:
...
case 1:
    fmt.Println("apple comes after banana")
}
```

[Compare](https://pkg.go.dev/strings#example-Compare) returns an integer comparing two strings lexicographically. The result will be:

- `0` if `a == b`
- `-1` if `a < b`
- And `+1` if `a > b`.

The [EqualFold]() function tells us if two strings are equal (case insensitive)
...
_ = ("Go" == "go")                 // false (case-sensitive)
_ = strings.EqualFold("Go", "go")   // true (case-insensitive)
```

## Sorting

For sorting strings the [sort](https://pkg.go.dev/sort) package comes in handy. The [Sort](https://pkg.go.dev/sort#Sort) function sorts data in **ascending order**:

```go
// In English, we expect: apple, café, zebra
// But byte-wise: apple, zebra, café (café comes last)
words := []string{"café", "zebra", "apple"}

sort.Strings(words)  // ["apple", "zebra", "café"] - incorrect for French!
```

This method uses byte-wise comparison, which doesn't respect locale-specific rules.

### Locale-Aware Sorting

For locale-aware sorting, use [golang.org/x/text](https://pkg.go.dev/golang.org/x/text):

```go
import "golang.org/x/text/collate"
import "golang.org/x/text/language"

// French-aware sorting
cl := collate.New(language.French)
cl.SortStrings(words)  // ["apple", "café", "zebra"]
```
