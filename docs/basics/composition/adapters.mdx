...
This is “verbose” because you create a named struct only to attach a single method that forwards to an existing function. The adapter avoids this.

### Using HandleFunc

[HandleFunc](https://pkg.go.dev/net/http#example-HandleFunc) is a helper function that takes your plain function and **internally** wraps it with `http.HandlerFunc` **adapter** before registering it. It's applying the adapter pattern for you behind the scenes.
...
Without the **adapter**, you'd need to create a struct for each handler:

```go
// Verbose: a wrapper type per function

type H1 struct{}
func (H1) ServeHTTP(w http.ResponseWriter, r *http.Request) { handlers.Home(w, r) }

type H2 struct{}
func (H2) ServeHTTP(w http.ResponseWriter, r *http.Request) { handlers.Contact(w, r) }

http.Handle("/", H1{})
http.Handle("/contact", H2{})
```

The adapter pattern saves you from this boilerplate.

## Using http.Handle (adapter)

[http.Handle](https://pkg.go.dev/net/http#Handle) expects something that satisfies the `http.Handler` interface (has a `ServeHTTP` method). Our functions don't have that method, so we use `http.HandlerFunc` to adapt them.
...
- `http.HandleFunc` - convenience function, takes plain functions directly
- `http.Handle` - takes `http.Handler` interface, requires adapter for plain functions
