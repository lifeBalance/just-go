...
## Real Example: The `net/http` package
...
Another option is to use the [http.HandleFunc](https://pkg.go.dev/net/http#example-HandleFunc) helper function, which takes your **plain function** and **internally** wraps it with the `http.HandlerFunc` **adapter** before calling `http.Handle` with it. 
