...
[http.HandleFunc](https://pkg.go.dev/net/http#example-HandleFunc) is a helper function that takes your plain function and **internally** wraps it with `http.HandlerFunc` **adapter** before registering it. It's applying the adapter pattern for you behind the scenes.
