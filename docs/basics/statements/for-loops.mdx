...
Some languages have a `do-while` loop that guarantees the loop body runs at least once before checking the condition. For example, in **JavaScript** (or TypeScript):
...
Go doesn't have a do-while loop, but you can achieve the same functionality (execute at least once, then check condition) using an infinite loop with a break:
...
A **do-while** checks the condition after each iteration (guarantees at least one execution), and that's exactly what the code above does: it runs the loop body first, then checks the condition to decide whether to break out of the loop.
