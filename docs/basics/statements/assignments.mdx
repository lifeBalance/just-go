...
In Go, [assignments](https://go.dev/ref/spec#Assignment_statements) are **statements** not expressions — they do not produce a value. This design avoids hidden side effects inside larger expressions. For example:
...
<Admonition variant="info" title="Assignments set or update variable values">
...
<Admonition variant="warning" title="Chained assignment vs Tuple assignment">
...
// Re‑use allowed if at least one new name appears on the left:
...
Assignments have no value; you can’t use them in conditions or other expressions.
...
<Admonition variant="tip" title="Why also in Expressions?">
  In Go, these operators are assignment statements (not expressions), so they
  don’t participate in operator precedence. We still discuss their operator part
