...
- Using an explicit syntax, using the keyword `var`.
- Using a shorthand syntax, using the `:=` operator.

In this section we'll go a bit deeper, and introduce new features related to variables and constants.

## Explicit Sintax

We can create variables in a two step process:
...
<Admonition>
When we use this two-step style, usually the assignment doesn't take place in the next line, but further in the program, according to our needs.
</Admonition>

If we know the value we want to initialize our variable with, we can do it in the same line:
...
- `''` for strings.
- `false` for booleans.
- `nil` for [interfaces](https://go.dev/ref/spec#Interface_types) and **reference types** (slice, pointer, map, channel, function).
