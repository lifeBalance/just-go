<script lang="ts">
  ...
  let { data } = $props<{ section: string; segment: string; path: string; nav: { label: string; href?: string; items: { url: string; title: string }[]; dir: string }[] }>()
</script>

<div class="flex h-[calc(100vh-var(--topbar-height))]">
  <aside class="sticky top-0 h-full overflow-y-auto w-64 shrink-0 bg-sd-hover">
    <DocsSidebar nav={data.nav} heading="" currentPath={data.path} />
  </aside>
  <main class="min-w-0 flex-1 bg-sd-bg p-6 overflow-y-auto">
    ...
    <div id="content">
      <DocsSectionPage section={data.section} segment={data.segment} />
    </div>
  </main>
  <aside class="hidden xl:block sticky top-0 h-full overflow-y-auto w-64 shrink-0 bg-sd-hover p-3 border-l border-sd-border">
    <DocsToc />
  </aside>
</div>
